...
def complex_create(request):
    if request.method == 'POST':
        if 'complex' in request.FILES:
            ...
            complex_name = os.path.basename(complex_filename)[:255]
            complex_string = request.FILES['complex'].read().decode('utf8')
            cmplx = Complex(name=complex_name, file_type=complex_extension[1:], file_string=complex_string,
            ...
            if 'ligand' in request.FILES:
                ...
                ligand_name = os.path.basename(ligand_filename)[:255]
                ligand_string = request.FILES['ligand'].read()
                ligand = Ligand(name=ligand_name, file_type=ligand_extension[1:], file_string=ligand_string,
                                complex_id=cmplx)
                ligand.save()
